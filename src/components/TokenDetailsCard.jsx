...
const TokenDetailsCard = ({
  ...
  const [purposeValue, setPurposeValue] = useState(token.purpose || '');
  const [counterpartValue, setCounterpartValue] = useState(token.counterpart || '');
      ...
      <CardContent className="p-6">
        ...
        <div className="d-flex gap-4 align-items-start mb-4">
          ...
          <img
            ...
            onError={(e) => {
              e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(token.name || token.ticker)}&size=128&background=random`;
            }}
          />
            ...
            {editingCounterpart ? (
              <>
                <Textarea
                  ...
                  onChange={(e) => setCounterpartValue(e.target.value)}
                  rows={4}
