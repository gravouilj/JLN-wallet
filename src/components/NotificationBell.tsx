...
interface NotificationItem {
  id: string;
  type: 'ticket' | 'verification' | 'transaction' | 'airdrop' | 'admin';
  title: string;
  subtitle: string;
  timestamp: Date;
  data: TicketData | FarmData | ActivityData;
  isRead: boolean;
}
...
const NotificationBell: React.FC<NotificationBellProps> = ({ compact = false }) => {
  ...
  const markAsRead = async (notif: NotificationItem): Promise<void> => {
    try {
      ...
      if (notif.type === 'ticket') {
        ...
        const activityData = notif.data as ActivityData;
        if (activityData.token_id) {
          navigate(`/token/${activityData.token_id}`);
        }
      }
