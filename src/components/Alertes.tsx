...
import { Card, CardContent, InfoBox, Button } from './UI';

/**
 * Composant d'alertes pour les profils utilisateur
 * À utiliser dans les pages de gestion de profil
 */

interface ProfileData {
  id?: string;
  name?: string;
  status?: 'active' | 'banned' | 'deleted' | 'pending' | null;
  verification_status?: 'none' | 'pending' | 'verified' | 'info_requested' | 'rejected' | null;
...
interface ProfileAlertsProps {
  ...
  checkVerificationAge?: () => number | null;
  handleConfirmInformation?: () => void;
  confirmingInfo?: boolean;
}

export const ProfileAlerts: React.FC<ProfileAlertsProps> = ({ existingProfile, checkVerificationAge, handleConfirmInformation, confirmingInfo }) => {
  ...
  return (
    <>
      ...
      {existingProfile.verification_status === 'rejected' && existingProfile.status !== 'banned' && existingProfile.status !== 'deleted' && (
      ...
      {/* ALERTE FERME BANNIE */}
      {(existingProfile.status === 'banned' || existingProfile.status === 'deleted') && (
        <InfoBox 
          type="error" 
          icon="🛑" 
      ...
      {checkVerificationAge && checkVerificationAge() !== null && (
        <Card style={{ 
