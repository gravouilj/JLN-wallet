...
import AddressBookMultiSelector from '../../AddressBookMultiSelector';
...
export const Message = ({
  ...
  const handleSendMessage = async ({ message, recipients }) => {
    ...
    if (!recipients || recipients.length === 0) {
      setNotification({ type: 'error', message: 'Veuillez spécifier au moins un destinataire' });
      return;
    }
    ...
    try {
      ...
      // Envoyer le message via OP_RETURN avec destinataires
      const result = await wallet.sendMessage(finalMessage, recipients);
  ...
  const onSubmit = (e) => {
    ...
    if (sendMode === 'single') {
      if (!recipient) {
        setNotification({ type: 'error', message: 'Veuillez spécifier un destinataire' });
        return;
      }
      recipients = [recipient];
    } else {
      // Mode multiple - parse la liste
      const lines = multipleRecipients.split('\n').map(l => l.trim()).filter(l => l.length > 0);
      if (lines.length === 0) {
        setNotification({ type: 'error', message: 'Veuillez spécifier au moins un destinataire' });
        return;
      }
  ...
  return (
    <div style={{ 
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1">
              <NetworkFeesAvail 
                compact={true} 
                showActions={true} 
                estimatedFee={250 + 150 + (6 + message.length) + 34 + (message.length > 100 ? 150 : 0)} 
              />
            </div>
          </div>
        )}
      ...
      <div style={{ marginTop: '32px', paddingTop: '24px', borderTop: '1px solid #e2e8f0' }}>
        ...
        ) : history.filter(h => h.action_type === 'MESSAGE').length > 0 ? (
          <HistoryList history={history.filter(h => h.action_type === 'MESSAGE')} compact />
        ) : (
          <div style={{ 
            textAlign: 'center', 
            padding: '32px', 
