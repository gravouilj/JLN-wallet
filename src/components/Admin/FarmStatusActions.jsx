...
/**
 * FarmStatusActions - Composant centralisé pour les actions de statut
 * Affiche uniquement les actions pertinentes selon l'état actuel de la ferme
 * 
 * @param {object} farm - Objet farm avec verification_status et status
 * @param {function} onStatusChange - Callback (farmId, newStatus)
 * @param {boolean} processing - État de chargement
 * @param {string} size - Taille des boutons ('sm' ou défaut)
 */
export const FarmStatusActions = ({ 
  farm, 
  onStatusChange, 
  processing = false,
  size = 'sm'
  ...
  const getAvailableActions = () => {
    const { verification_status, status } = farm;
    ...
    <ActionBar title="⚡ Actions rapides">
      {actions.map((action, idx) => (
        <Button
          ...
          size={size}
          onClick={() => onStatusChange(farm.id, action.action)}
          disabled={processing}
          style={{
