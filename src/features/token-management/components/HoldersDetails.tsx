...
interface HoldersDetailsProps {
  holdersCount: number | null;
  calculatedHolders?: HolderData[];
  tokenId: string;
  setNotification: (notification: NotificationPayload) => void;
  onHoldersUpdate?: () => void;
}

/**
 * HoldersDetails - Affiche la liste détaillée des détenteurs éligibles
 */
const HoldersDetails: React.FC<HoldersDetailsProps> = ({ 
  holdersCount, 
  calculatedHolders = [], 
  tokenId, 
  setNotification,
  ...
  const handleSaveContact = (address: string): void => {
    if (contactName.trim()) {
  ...
  const handleDeleteContact = (address: string, displayName: string): void => {
    ...
    <div style={{ backgroundColor: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
      <div style={{ padding: '12px 16px', backgroundColor: '#f1f5f9', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          ...
          <button
            ...
            onMouseEnter={(e: MouseEvent<HTMLButtonElement>) => e.currentTarget.style.backgroundColor = '#2563eb'}
            onMouseLeave={(e: MouseEvent<HTMLButtonElement>) => e.currentTarget.style.backgroundColor = '#3b82f6'}
          >
            📥 Exporter CSV
          </button>
        </div>
      ...
      {calculatedHolders.length > 0 && (
        <div style={{ maxHeight: '400px', overflowY: 'auto', padding: '8px' }}>
          {calculatedHolders.map((holder, idx: number) => {
