...
import ClientTicketForm from '../../../support/components/ClientTicketForm';
import { useEcashWallet } from '../../../../hooks/useEcashWallet';
import { UserProfile, TokenDataFromProfile } from '../../../../types';
...
interface CreatorProfileCardProps {
  profile: UserProfile;
  profileTickers?: Record<string, string>;
  onCardClick: (profile: UserProfile) => void;
}
...
const CreatorProfileCard = memo<CreatorProfileCardProps>(({ profile, profileTickers = {}, onCardClick }) => {
  ...
  // Get visible tokens
  const visibleTokens: TokenDataFromProfile[] = profile.tokens?.filter(token => token.isVisible) || [];
  const primaryToken = visibleTokens[0] as TokenDataFromProfile | undefined;
  ...
  const certifications = (profile.certifications || {}) as Certifications;
  const nationalCert = certifications.national;
  const internationalCert = certifications.international;
  
  // Get country flag emoji
  const getCountryFlag = useCallback((country: string | undefined): string => {
  ...
  const getFrenchDepartmentCode = useCallback((postalCode: string | undefined): string | null => {
  ...
  const handlePayClick = useCallback((e: MouseEvent<HTMLButtonElement>): void => {
    e.stopPropagation();
    if (primaryToken) {
  ...
  const handleSelectClick = useCallback((e: MouseEvent<HTMLButtonElement>): void => {
    e.stopPropagation();
    onCardClick(profile);
  }, [onCardClick, profile]);
  
  const handleFavoriteClick = useCallback((e: MouseEvent<HTMLButtonElement>): void => {
    e.stopPropagation();
    if (walletConnected) {
      ...
      <div style={{ padding: '20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: hasBalance ? '1fr 1fr' : '1fr', gap: '8px' }}>
          <button
            ...
            onMouseEnter={(e: MouseEvent<HTMLButtonElement>) => (e.target as HTMLButtonElement).style.opacity = '0.9'}
            onMouseLeave={(e: MouseEvent<HTMLButtonElement>) => (e.target as HTMLButtonElement).style.opacity = '1'}
          >
            Voir le profil
          </button>
          {hasBalance && (
...
}, (prevProps: CreatorProfileCardProps, nextProps: CreatorProfileCardProps) => {
