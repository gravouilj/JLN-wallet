...
import { StatusBadge, Badge, Modal, Button, Textarea } from '../../../../components/UI';
...
const CreatorProfileModal: React.FC<CreatorProfileModalProps> = ({ profile, isOpen, onClose, profileTickers = {} }) => {
  ...
  const [selectedContactToken, setSelectedContactToken] = useState<TokenDataFromProfile | null>(null);
  ...
  const tokenBalances: Record<string, string | number | null> = {};
  ...
  const getFrenchDepartmentCode = (postalCode: string | undefined): ZipInfo | null => {
  ...
  const toggleTokenExpand = (tokenId: string): void => {
  ...
  const zipInfo: ZipInfo | null = profile.location_country === 'France' ? getFrenchDepartmentCode(profile.postal_code) : null;
